		...
		_transform = function(self, value, state)
			...
			if self.t ~= got then
				return FailedTransform,
					"expected type " .. tostring(describe_type(self.t)) .. ", got " .. tostring(describe_type(got))
			end
			if self.length_type then
				...
				if res == FailedTransform then
					return FailedTransform,
						tostring(self.t) .. " length " .. tostring(state) .. ", got " .. tostring(len)
				end
			end
			return value, state
		end,
	...
	local _base_0 = {
		...
		_transform = function(self, value, state)
			...
			for i, v in pairs(value) do
				...
				if not (i == k) then
					return FailedTransform,
						"non array index, got " .. tostring(describe_type(i)) .. " but expected " .. tostring(
							describe_type(k)
						)
				end
				k = k + 1
			...
			for idx, item in ipairs(value) do
				...
				if is_literal then
					if self.expected ~= item then
						return FailedTransform,
							"array item " .. tostring(idx) .. ": expected " .. tostring(describe_type(self.expected))
					else
...
do
	...
	local _base_0 = {
		_describe = function(self)
			return "map of "
				.. tostring(self.expected_key:_describe())
				.. " -> "
				.. tostring(self.expected_value:_describe())
		end,
		_transform = function(self, value, state)
			local pass, err = types.table(value)
			if not pass then
				...
				repeat
					...
					if value_literal then
						if v ~= self.expected_value then
							return FailedTransform,
								"map value expected " .. tostring(describe_type(self.expected_value))
						end
					else
	...
	_class_0 = setmetatable({
		__init = function(self, pattern, opts)
			...
			if opts then
				...
				return assert(
					opts.initial_type == nil,
					"initial_type has been removed from types.pattern (got: " .. tostring(opts.initial_type) .. ")"
				)
			end
		end,
		__base = _base_0,
